...
- **`service-projects`:**
  -  Creates individual service accounts for Prod and Non Prod environments. Learn more.
- **`vpn`:**
  - Creates hybrid connectivity to your on premises networks using a optional highly available VPN.
- **`log-export`:**
  - Deploys a centralized project for collecting and analyzing logs and metrics from other projects, enhancing visibility and troubleshooting capabilities. [Learn more](https://cloud.google.com/logging/docs/overview) and [Learn more](https://cloud.google.com/monitoring/docs/overview).
- **`security`:** 
  - Deploys a security policies. Org policies in GCP provide centralized control and enforcement of configurations across an entire organization. Security command center provides visibility into your cloud assets, detects threats, and helps you take action to mitigate risks..
...
- **Terraform:** Install Terraform (version 1.5.1 or later) to manage your infrastructure as code. [Download Terraform](https://www.terraform.io/downloads.html).
- **gcloud:** Install the gcloud command line utility as per the [instructions list here](https://cloud.google.com/sdk/docs/install-sdk).
-  If you did not configure a billing account admin during the initial setup, see the Boostrap section in the appendix.

## State File Handling

This blueprint utilizes a Google Cloud Storage (GCS) backend to store the Terraform state file.
The state file is created automatically if you used a billing account enabled before generating this blueprint in the Cloud Setup UI. If you did not specify a billing account, or the state file does not exist see the Bootstrap step in the Appendix.

## Committing to Git repository of choice.
Code should be committed to a git repository such as GCP Secure Source Manager,  GitHub, AzureDevOps or similar depending on your needs.
...
2. **Update Variables:** Verify the `cloud-setup.auto.tfvars` file contains your desired organization ID and billing account number.
  ```terraform
...
This option leverages Workload Identity Federation to allow your local machine or Cloud Shell to impersonate a service account with the necessary permissions to deploy your Terraform infrastructure. This approach aligns with the principle of least privilege, as recommended by Google Cloud's security best practices (https://cloud.google.com/security/best-practices).
...
2. **Grant Necessary IAM Permissions:**
   - Grant permission for your user to impersonate the newly created service account
   ```
   ...
   - Assign Required Roles:
      ...
      OR Assign roles at the folder level
      ```
...
3. **Configure Workload Identity Federation (if using a local machine):**
- Follow the instructions to configure Workload Identity Federation between your local environment and GCP: https://cloud.google.com/iam/docs/configuring-workload-identity-federation

4. **Impersonate the Service Account:**
```bash
gcloud iam service-accounts impersonate terraform-deployer@PROJECT_ID.iam.gserviceaccount.com --project=PROJECT_ID
```
...
- **Terraform State Management:** Store your Terraform state remotely in a secure location like Google Cloud Storage and enable encryption at rest. (https://cloud.google.com/docs/terraform/best-practices#manage-terraform-state)
...
2. **Folder for VPN Host Projects (For Hybrid Connectivity):** When utilizing hybrid connectivity, such as VPN, a dedicated folder is used to store secrets related to VPN connectivity in Secret Manager, improving security and centralizing sensitive information.


   ```bash
   # Create the folder
   FOLDER_ID="vpn-host-projects"
   gcloud resource-manager folders create --display-name="VPN Host Projects" --id=$FOLDER_ID


   # Grant the Terraform service account the necessary permissions on the folder
   # Replace "terraform-sa@$PROJECT_ID.iam.gserviceaccount.com" with your Terraform service account's email
   gcloud resource-manager folders add-iam-policy-binding $FOLDER_ID \
   --member="serviceAccount:terraform-sa@$PROJECT_ID.iam.gserviceaccount.com" \
   --role="roles/resourcemanager.folderAdmin"


   # Grant the Terraform service account the Secret Manager Secret Accessor role on the folder
   gcloud resource-manager folders add-iam-policy-binding $FOLDER_ID \
   --member="serviceAccount:terraform-sa@$PROJECT_ID.iam.gserviceaccount.com" \
   --role="roles/secretmanager.secretAccessor"
   ```

**Note:** Replace `"vpn-host-projects"` with your desired folder ID and `"terraform-sa@$PROJECT_ID.iam.gserviceaccount.com"` with your Terraform service account's email address, or email address of administrator running terraform. See the "Deployment" Section for details on the service account.
